...
- 🎯 **Drop-in FFmpeg Replacement**: Full FFmpeg syntax support with automatic passthrough for unsupported operations
...
This tool is currently in **alpha stage** and provided **as-is without warranty**. While functional as an MVP, further refinement is needed for production use, particularly for integration with video processing pipelines like Jellyfin, Plex, Emby, or Stremio.
...
- **Features**: Full FFmpeg syntax support (`-i`, `-map`, `-codec`, `-ss`, etc.) + RTX processing
...
- **Integration**: Enables use in media servers (Jellyfin, Plex, Emby, Stremio) as FFmpeg replacement
