...
public:
    ...
    explicit TimestampManager(const Config& config)
        : cfg_(config)
        , needs_baseline_wait_(config.input_seek_us > 0 && config.mode == Mode::NORMAL)
...
private:
    ...
    void adjustNormalPacket(AVPacket* pkt, AVRational in_tb, int stream_index) {
        if (!needs_baseline_wait_) return;
...
private:
    ...
    void validatePacketTimestamps(AVPacket* pkt) {
        if (!cfg_.enable_validation) return;
    ...
    const char* getAvoidNegativeTsName(AvoidNegativeTs mode) const {
        switch (mode) {
            case AvoidNegativeTs::AUTO: return "auto";
            case AvoidNegativeTs::MAKE_ZERO: return "make_zero";
            case AvoidNegativeTs::MAKE_NON_NEGATIVE: return "make_non_negative";
            case AvoidNegativeTs::DISABLED: return "disabled";
