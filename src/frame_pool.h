...
public:
    ...
    void initialize(AVBufferRef *hw_frames_ctx, int width, int height, int pool_size) {
        if (!hw_frames_ctx) throw std::runtime_error("CudaFramePool: hw_frames_ctx is null");
        m_hw_frames_ctx = av_buffer_ref(hw_frames_ctx);
        if (!m_hw_frames_ctx) throw std::runtime_error("CudaFramePool: av_buffer_ref failed");
        m_frames.clear();
        m_frames.reserve(pool_size);
        for (int i = 0; i < pool_size; ++i) {
            FramePtr f(av_frame_alloc(), &av_frame_free_single_fp);
            if (!f) throw std::runtime_error("CudaFramePool: av_frame_alloc failed");
            ...
            if (err < 0) throw std::runtime_error("CudaFramePool: av_hwframe_get_buffer failed during init");
    ...
    AVFrame* acquire() {
        if (m_frames.empty()) throw std::runtime_error("CudaFramePool not initialized");
        ...
        if (err < 0) throw std::runtime_error("CudaFramePool: av_hwframe_get_buffer failed in acquire");
        return slot.get();
    }
